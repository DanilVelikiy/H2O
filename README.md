...
```sh
sudo docker build -t jypiter_h2o .
```

Запуск службы без перезапуска

```sh
sudo docker run -p 4800:8888 --mount type=bind,source="$(pwd)"/data,target=/home/jupyter jypiter_h2o
```

Запуск службы в фоне в режиме с перезапуском

```sh
sudo docker run -d --restart always -p 4800:8888 --mount type=bind,source="$(pwd)"/data,target=/home/jupyter jypiter_h2o
```

Открыть блокнот

```sh
http://192.168.1.100:4800/lab/
```
